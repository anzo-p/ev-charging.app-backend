    ...
    Http.collectZIO[Request] {
      case req @ Method.POST -> !! / "api" / "chargers" / "start" =>
        (for {
          ...
          sessionId <- chargingService.initialize(session).mapError(th => badRequest(th.getMessage))
          _         <- toBackend.put(session.copy(sessionId = sessionId).toChargingEvent).mapError(serverError)
      ...
      case Method.GET -> !! / "api" / "chargers" / "session" / id =>
        (for {
          ...
          session   <- chargingService.get(sessionId).mapError(th => badRequest(th.getMessage))
        } yield {
          Response(
            Status.Ok,
            defaultHeaders,
